...
c = 'lightblue' if g == 'M' else 'pink'
dot.node(earl_ans, earl_ans, tooltip = det, shape = 'rect', style='filled', fillcolor=c)
    ...
    if len(temp) == 0:      # Break loop when all individuals have been recorded
        ...
        if len(this_gen) > 0:
            for j in range(0, len(this_gen)):
                ...
                with dot.subgraph() as subs:
                    subs.attr(rank = 'same')
                    subs.node(per1, per1, tooltip = det, shape = 'rect', style='filled', fillcolor=c)
                    node_nm.append(per1)
                    subs.edge(per2, per1, arrowhead = 'none')
        ...
        if len(this_gen) > 0:
            for j in range(0, len(this_gen)):
                ...
                g = this_gen['Gender'].iloc[j]
                c = 'lightblue' if g == 'M' else 'pink'
                dot.node(per1, per1, tooltip = det, shape = 'rect', style='filled', fillcolor=c)
                node_nm.append(per1)
                dot.edge(per2, per1)
